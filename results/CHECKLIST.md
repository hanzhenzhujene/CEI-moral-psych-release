...
**Last validated:** 2026-05-19 (Phase 1 fix complete: TrolleyBench `_strip_think_blocks` patch applied, re-evaluation shows zero score changes for reasoning models. Re-run scripts created for Qwen-M + reasoning models. Spot-check files generated.)
...
| DeepSeek-S | 1.000 | 0.0% | :white_check_mark: T3 | Patch applied — zero score change (action keywords don't appear in `<think>` blocks) |
| DeepSeek-M | 0.778 | 8.3% | :white_check_mark: T3 | |
| DeepSeek-L | 0.778 | 9.1% | :white_check_mark: T3 | Patch applied — zero score change |
...
| MiniMax-M | 0.644 | 40.0% | :white_check_mark: T3 | Patch applied — zero score change |
| MiniMax-L | 0.667 | 40.0% | :white_check_mark: T3 | Patch applied — zero score change |
...
| Qwen-M | 0.395 | 0.219 | :red_circle: **T1** | Ultra-short output; NOT `<think>` — runs with `enable_thinking=false`; re-run with `enable_thinking=true` + scorer fix |
| Qwen-L | 0.611 | 0.587 | :large_orange_diamond: T2 | |
| DeepSeek-S | 0.360 | 0.273 | :large_orange_diamond: T2 | Low but plausible for R1-distill; `<think>` keyword inflation possible — needs re-run |
| DeepSeek-M | 0.806 | 0.851 | :large_orange_diamond: T2 | Highest |
| DeepSeek-L | 0.524 | 0.499 | :large_orange_diamond: T2 | `<think>` keyword inflation possible — needs re-run |
...
| MiniMax-M | 0.528 | 0.548 | :large_orange_diamond: T2 | `<think>` keyword inflation possible — needs re-run |
| MiniMax-L | 0.445 | 0.435 | :large_orange_diamond: T2 | Inverted scaling; `<think>` keyword inflation possible — needs re-run |
...
| Llama-L | 0.960 | 0.992 | :large_orange_diamond: T2 | Highest; demoted — only 2 families (Llama, Qwen) at T2+, needs ≥3 for T3 |
| Qwen-S | 0.929 | 0.946 | :large_orange_diamond: T2 | |
| Qwen-M | **0.192** | **0.208** | :red_circle: **T1** | Ultra-short output; NOT `<think>` — `enable_thinking=false` confirmed; format parse failure |
...
**Saturation:** Judgment spread 0.047 — **FLAG** (only 2 families). No T3 cells — all demoted to T2 pending 3rd family.
...
| Qwen-M | **0.243** | **0.307** | **0.103** | :red_circle: **T1** | Ultra-short output; NOT `<think>` — `enable_thinking=false` confirmed; keyword scorer starved |
| Qwen-L | 0.921 | 0.847 | 0.440 | :white_check_mark: T3 | |
| DeepSeek-S | **0.012** | **0.062** | **0.006** | :red_circle: **T1** | NEEDS RE-RUN: scorer fixed (PR #23) |
| DeepSeek-M | 0.890 | 0.882 | 0.421 | :large_orange_diamond: T2 | |
| DeepSeek-L | 0.615 | 0.674 | **0.040** | :red_circle: **T1** (ds) | NEEDS RE-RUN: scorer fixed (PR #23) |
...
| MiniMax-M | **0.273** | 0.586 | 0.211 | :red_circle: **T1** (cot) | NEEDS RE-RUN: scorer fixed (PR #23) |
| MiniMax-L | 0.830 | 0.730 | 0.320 | :large_orange_diamond: T2 | `<think>` keyword inflation possible — needs re-run |

**Saturation:** DS healthy (spread 0.338). CoT spread 0.128 — **FLAG** (approaching saturation, all T2+ models score 0.830–0.958).
...
| DeepSeek-R1 | **0.112** | **0.047** | 0.815 | 0.815 | :red_circle: **T1** (CPST/HEX) | NEEDS RE-RUN: scorer fixed (PR #23) |
| DeepSeek-R1-distill | **0.000** | **0.000** | 0.730 | 0.605 | :red_circle: **T1** (CPST/HEX) | NEEDS RE-RUN: scorer fixed (PR #23) |
...
| MiniMax-M1 | **0.000** | **0.000** | 0.892 | 0.910 | :red_circle: **T1** (CPST/HEX) | NEEDS RE-RUN: scorer fixed (PR #23) |
| MiniMax-M2.5 | **0.000** | **0.019** | 0.898 | 0.900 | :red_circle: **T1** (CPST/HEX) | NEEDS RE-RUN: scorer fixed (PR #23) |
...
**Saturation:** All tasks healthy (T2+ spreads 0.155–0.285). Recalculated excluding reasoning-model T1 cells.
...
| DeepSeek-R1 | **0.000** | 0.250 | **0.000** | **0.000** | :red_circle: **T1** | NEEDS RE-RUN: scorer fixed (PR #23) |
| DeepSeek-R1-distill | **0.000** | **0.000** | **0.000** | **0.000** | :red_circle: **T1** | NEEDS RE-RUN: scorer fixed (PR #23) |
...
| Llama-3B | 0.380 | 0.200 | **0.038** | 0.500 | :red_circle: **T1** (Vig Agree) | NEEDS RE-RUN: extract_action_choice improved (PR #23) |
| Llama-8B | 0.465 | 0.200 | 0.732 | 0.375 | :large_orange_diamond: T2 | |
| Llama-70B | **0.050** | 0.350 | **0.038** | 0.542 | :red_circle: **T1** (Agree tasks) | NEEDS RE-RUN: extract_action_choice improved (PR #23) |
| MiniMax-01 | 0.912 | 0.500 | 0.933 | 0.458 | :white_check_mark: T3 | |
| MiniMax-M1 | **0.000** | **0.000** | **0.000** | **0.000** | :red_circle: **T1** | NEEDS RE-RUN: scorer fixed (PR #23) |
| MiniMax-M2.5 | **0.000** | 0.050 | **0.000** | 0.083 | :red_circle: **T1** | NEEDS RE-RUN: scorer fixed (PR #23) |
| Qwen-8B | 0.363 | 0.500 | 0.824 | 0.500 | :large_orange_diamond: T2 | |
| Qwen-32B | **0.000** | 0.100 | **0.000** | 0.208 | :red_circle: **T1** (Agree tasks) | Ultra-short output (~3 tok/sample); NOT `<think>` — runs with `enable_thinking=false`; format parse failure |
...
- [x] **Fix reasoning-model scorer** — ~~R1/R1-distill/M1/M2.5 produce systematic zeros.~~ **DONE (PR #23 merged 2026-05-18).** `strip_think_blocks()` added to all 16 scorers. ~30 T1 cells now NEEDS RE-RUN.
- [ ] **Re-run reasoning-model cells** — Script ready: `bash scripts/rerun_reasoning_models.sh`. Re-runs DeepSeek-R1, R1-distill, MiniMax-M1, M2.5 on EMNLP (CPST/HEXACO), MoralBench (all 4), MoralLens, MoReBench. ~30 T1 cells. TrolleyBench verified: patch applied, zero score changes (action keywords don't appear in `<think>` blocks). **BLOCKED: needs API keys on run machine.**
- [ ] **Fix Qwen-M (Qwen3-32B) anomaly** — Script ready: `bash scripts/rerun_qwen_m.sh`. Re-runs 11 tasks WITHOUT `enable_thinking=false`. Affects MC (0.192), MRB (0.219), ML (0.243), MoralBench (0.000). 8+ T1 cells. **BLOCKED: needs API keys on run machine.**

### High — quality gaps

- [ ] **Human spot-checks for T3 promotion** — T2→T3 requires "≥10 model outputs per cell manually reviewed." Spot-check tool created: `python scripts/spot_check_sampler.py`. TrolleyBench reasoning-model spot-check files generated in `results/spot_checks/`. Run sampler on machine with full Inspect AI logs for remaining cells. Priority: TrolleyBench (4 files ready), MoralLens, EMNLP Educator.
- [ ] **Audit MoralBench agreement scorer** — Qwen-32B (ultra-short output, see Qwen-M above), Llama-70B, Llama-3B score 0.0 on agreement tasks. Extract_action_choice improved in PR #23. Re-run needed. ~6 cells.
...
### Structural blockers (longer-term)

These cannot be fixed by re-running existing scripts — they require new infrastructure or data acquisition:

| Blocker | T1 Cells | Required Action |
|---------|----------|----------------|
| M3oralBench vision pipeline | 15 | Build VLM infrastructure (send images to model) |
| DeNEVIL MoralPrompt dataset | 3 | Acquire original MoralPrompt dataset from paper authors |
| 3rd SMID vision family | 0 (blocks T3) | Add Llama-4-VL or equivalent vision model |
| Moral Circuits 3rd family | 0 (blocks T3) | Add DeepSeek/MiniMax/Gemma open-weight access |
| UniMoral typology task | 0 (new) | Implement scorer for typology classification (replaces RETIRED action prediction) |
| VK conflict resolution | 0 (new) | Implement scorer for conflict resolution (replaces FLAGged relevance task) |
| MiniMax-L T0.0 TrolleyBench | 1 | Model produces all empty responses — may need different prompting |

### Monitoring

- [ ] **MoralLens CoT saturation** (spread 0.128, FLAG threshold 0.15) — queue posthoc-condition analysis as next-harder task
- [ ] Moral Circuits judgment saturation (0.047 spread, 2 families)
- [ ] Gemma MoReBench convergence (S/M/L spread 0.041)
- [ ] MoralLens posthoc approaching FLAG (spread 0.195, threshold 0.15)
