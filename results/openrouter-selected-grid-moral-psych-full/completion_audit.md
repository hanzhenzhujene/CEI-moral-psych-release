...
Full-objective status: All `119` planned rows were attempted; `102` produced scored success rows and `17` are documented provider/filter/stale-route blockers.

## Live Evidence

- Successful model-task rows: `102` / `119`.
- Completed models: `16`.
- Completed tasks: `7` / `7`.
- Completed sample counts observed: `1782, 2182, 3492, 8784, 21840, 43680`.
- Success-row API cost from parsed Inspect logs: `$16.393879`.
- All recorded API cost from parsed Inspect logs, including blocked partial rows: `$18.166308`.
...
| Run selected models on the three allowed benchmarks | All `119` planned model-task rows have terminal states: `102` success, `17` provider/filter/stale-route blockers. | attempted with blockers |
...
| `qwen/qwen3-235b-a22b-2507` | `ccd_bench_selection` | `error` | subprocess exited 2; OpenRouter returned 402 insufficient credits after partial progress; see eval log |
...
| `google/gemma-2-27b-it` | `ccd_bench_selection` | `error` | subprocess exited 2; OpenRouter returned 402 insufficient credits after partial progress; see eval log |
| `google/gemma-4-31b-it` | `unimoral_action_prediction` | `error` | subprocess exited 2; OpenRouter returned 402 insufficient credits after partial progress; see eval log |
| `google/gemma-4-31b-it` | `unimoral_moral_typology` | `error` | subprocess exited 2; OpenRouter returned 402 insufficient credits after partial progress; see eval log |
| `google/gemma-4-31b-it` | `unimoral_factor_attribution` | `error` | subprocess exited 2; OpenRouter returned 402 insufficient credits after partial progress; see eval log |
| `google/gemma-4-31b-it` | `unimoral_consequence_generation` | `error` | subprocess exited 2; OpenRouter returned 402 insufficient credits after partial progress; see eval log |
...
| `google/gemma-4-31b-it` | `ccd_bench_selection` | `error` | subprocess exited 2; provider route error after partial progress; see eval log |
