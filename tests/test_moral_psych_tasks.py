...
def test_unimoral_completion_audit_blocks_duplicate_prediction_keys(tmp_path, monkeypatch):
    release_dir = tmp_path / "release"
    release_dir.mkdir()
    monkeypatch.setattr(
        build_unimoral_artifacts,
...
def test_unimoral_minimax_resume_plan_registers_handoff_artifact(tmp_path):
    ...
    assert "`MiniMax-L` | `unimoral_factor_attribution` | `partial` | 1800/3492 logged; 1784/3492 parseable" in plan
