...
For paper-calibration claims, use [`paper-result-comparison.md#safe-citation-matrix`](paper-result-comparison.md#safe-citation-matrix). It separates exact same-model numeric bars, UniMoral metric-bridged calibration, CCD behavior, ValuePrism/Kaleido boundaries, and DeNEVIL proxy-only evidence.
