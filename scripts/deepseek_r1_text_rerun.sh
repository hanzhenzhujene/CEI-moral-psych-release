...
run_task() {
  ...
  # The OpenRouter OpenAI-compatible path does not accept the same direct
  # DeepSeek client kwargs. Keep the no-think prompt prefix, and use
  # OpenRouter's normalized reasoning controls instead so R1 does not reserve a
  # large hidden thinking budget and trip 402s before producing output.
  ...
  if (
    ...
    if [[ "$ROUTED_PROVIDER" == "openrouter" && -n "${OPENROUTER_API_KEY:-}" ]]; then
...
run_family() {
  ...
  if ! run_task "$family" "ccd_bench_selection" "src/inspect/evals/ccd_bench.py::ccd_bench_selection" "$DEEPSEEK_TEXT_MODEL" "$TEXT_MAX_CONNECTIONS"; then
    rm -f "$run_dir/family_done.txt"
    return $?
  fi
  if ! run_task "$family" "unimoral_action_prediction" "src/inspect/evals/unimoral.py::unimoral_action_prediction" "$DEEPSEEK_TEXT_MODEL" "$TEXT_MAX_CONNECTIONS"; then
    rm -f "$run_dir/family_done.txt"
    return $?
  fi
  if ! run_task "$family" "denevil_fulcra_proxy_generation" "src/inspect/evals/denevil.py::denevil_fulcra_proxy_generation" "$DEEPSEEK_TEXT_MODEL" 4; then
    rm -f "$run_dir/family_done.txt"
    return $?
  fi
  if ! run_task "$family" "value_prism_valence" "src/inspect/evals/value_kaleidoscope.py::value_prism_valence" "$DEEPSEEK_TEXT_MODEL" "$TEXT_MAX_CONNECTIONS"; then
    rm -f "$run_dir/family_done.txt"
    return $?
  fi
  if ! run_task "$family" "value_prism_relevance" "src/inspect/evals/value_kaleidoscope.py::value_prism_relevance" "$DEEPSEEK_TEXT_MODEL" "$TEXT_MAX_CONNECTIONS"; then
    rm -f "$run_dir/family_done.txt"
    return $?
  fi
...
launch_family() {
  local family="$1"
  local pidfile stdout_path existing_pid session_name bootstrap_path launched_pid
  pidfile="$PID_DIR/$family.pid"
  stdout_path="$LAUNCHER_STDOUT_DIR/${family}.out"
  session_name="${INSTANCE_NAME:-$family}-${RUN_ID//[^[:alnum:]]/_}"
  bootstrap_path="$LAUNCHER_STDOUT_DIR/${family}.bootstrap.sh"
...
set -euo pipefail
echo \$\$ > "$pidfile"
exec /bin/bash "$SCRIPT_PATH" run "$family" > "$stdout_path" 2>&1
EOF
  chmod +x "$bootstrap_path"

  screen -S "$session_name" -X quit >/dev/null 2>&1 || true
  rm -f "$pidfile"
  screen -dmS "$session_name" /bin/bash "$bootstrap_path"
  sleep 1

  launched_pid="$(cat "$pidfile" 2>/dev/null || true)"
  if [[ -z "$launched_pid" ]] || ! is_running_pid "$launched_pid"; then
    echo "$family launch failed"
    return 1
  fi

  echo "$family launched (pid $launched_pid)"
}
