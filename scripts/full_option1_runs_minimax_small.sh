...
task_progress_log_dirs() {
  ...
  if [[ "$family" == "minimax_text" && "$task_name" == "value_prism_relevance" ]]; then
    local relevance_concurrent_log_dir="${MINIMAX_RELEVANCE_CONCURRENT_LOG_DIR:-}"
    if [[ -n "$relevance_concurrent_log_dir" && -d "$relevance_concurrent_log_dir" && "$relevance_concurrent_log_dir" != "$primary_log_dir" ]]; then
