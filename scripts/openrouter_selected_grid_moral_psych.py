#!/usr/bin/env python3
"""OpenRouter selected-grid planning/running for Jenny's text-only moral-psych tasks.
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "results" / "openrouter-selected-grid-moral-psych"
...
MODEL_SPECS: list[ModelSpec] = [
    ...
    ModelSpec("deepseek/deepseek-v4-flash", "DeepSeek", "flash/V4", "2026-Q2", "time scaling", "Newest DeepSeek flash route"),
]
...
def render_cost_plot(rows: list[dict[str, Any]], output_path: Path) -> None:
    ...
    plt.title("OpenRouter Selected-Grid Moral-Psych Run Plan")
    plt.tight_layout()
...
def parse_run_results(plan_rows: list[dict[str, Any]], models_by_id: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in plan_rows:
        ...
        if result.get("status") == "unreadable" and row.get("run_status") not in {"", "planned", "unknown"}:
...
def summarize_patterns(
    ...
    if sample_limit is None and result_rows:
        title = "# OpenRouter Full Selected-Grid Interpretation"
        evidence_sentence = (
        ...
        title = "# OpenRouter Selected-Grid Interpretation"
...
def write_completion_audit(
    ...
    if sample_limit is None and result_rows and completed and len(completed) == len(result_rows):
        ...
        full_estimate_path = PROJECT_ROOT / "results" / "openrouter-selected-grid-moral-psych-full-estimate" / "run_plan.csv"
    ...
    full_dry_run_command = "OPENROUTER_FULL_RUN_DRY_RUN=1 scripts/run_openrouter_selected_grid_full.sh"
    full_run_command = "OPENROUTER_FULL_RUN_APPROVED=1 scripts/run_openrouter_selected_grid_full.sh"
    ...
    if result_rows is None or sample_limit is not None or not all_planned_rows_recorded:
        lines.extend(
            [
                ...
                "```",
                "",
                "The guarded launcher keeps the live run bounded by `OPENROUTER_MAX_TOTAL_ESTIMATED_COST=60` by default, uses `OPENROUTER_MAX_CONNECTIONS=1` for provider stability, writes to `results/openrouter-selected-grid-moral-psych-full`, and keeps completed rows resumable through the planner's default `--skip-existing-success` behavior.",
                "",
                (
