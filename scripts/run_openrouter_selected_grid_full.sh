...
OUTPUT_DIR="${OPENROUTER_FULL_OUTPUT_DIR:-results/openrouter-selected-grid-moral-psych-full}"
MAX_TOTAL_ESTIMATED_COST="${OPENROUTER_MAX_TOTAL_ESTIMATED_COST:-60}"
MAX_CONNECTIONS="${OPENROUTER_MAX_CONNECTIONS:-1}"

RUN_CMD=(
  "$PYTHON_BIN"
  scripts/openrouter_selected_grid_moral_psych.py
  run
  --full
  --output-dir
  "$OUTPUT_DIR"
...
echo "OpenRouter full selected-grid run"
